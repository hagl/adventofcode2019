...
stringToPath :: String -> (Int, Int, [(Int, Int)])
stringToPath str =
  let moves = splitOn "," str
  in foldl move (0, 0, [(0, 0)]) moves

move :: (Int, Int, [(Int, Int)]) -> String -> (Int, Int, [(Int, Int)])
move (x, y, acc) turn =
  let count = read $ tail turn :: Int
  in case head turn of
       'R' -> (x + count, y, acc ++ map (\i -> (x + i, y)) [1 .. count])
       'L' -> (x - count, y, acc ++ map (\i -> (x - i, y)) [1 .. count])
       'U' -> (x, y + count, acc ++ map (\i -> (x, y + i)) [1 .. count])
       'D' -> (x, y - count, acc ++ map (\i -> (x, y - i)) [1 .. count])

test =
  let (_, _, l1) = stringToPath s1
      (_, _, l2) = stringToPath s2
      list
        -- intersectBy (\(x, _) (y, _) -> x == y) (zip l1 [0 ..]) (zip l2 [0 ..])
       =
        intersectBy
          (\(x, _) (y, _) -> x == y)
          (take 32064 $ zip l1 [0 ..])
          (take 32064 $ zip l2 [0 ..])
  -- in list
  -- in (map (\(x, y) -> (abs x) + (abs y)) list)
      -- elem = head (tail list)
  -- in liftA2 (+) (elemIndex elem l1) (elemIndex elem l2)
      list2 = map (\((x, y), p1) -> p1 + (fromJust (elemIndex (x, y) l2))) list
  in head $ tail $ sort list2
